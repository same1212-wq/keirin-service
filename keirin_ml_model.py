...
def build_features(entries, results, lines, races):
    ...
    df_e = df_e.merge(df_race[["race_id","grade","race_type","player_count"]], on="race_id", how="left", suffixes=("","_race"))
    line_map = {}
    ...
    race_stats = df_e.groupby("race_id").agg(score_mean=("current_score","mean"), score_std=("current_score","std"), score_max=("current_score","max")).reset_index()
    ...
    df_e["nige_rate"]      = df_e.apply(lambda r: r["kimete_nige"]/max(r["kimete_total"],1), axis=1)
    df_e["makuri_rate"]    = df_e.apply(lambda r: r["kimete_makuri"]/max(r["kimete_total"],1), axis=1)
    df_r["is_top3"] = (df_r["rank"]<=3).astype(int)
    df_r["is_top1"] = (df_r["rank"]==1).astype(int)
    df = df_e.merge(df_r[["race_id","car_no","rank","is_top3","is_top1"]], on=["race_id","car_no"], how="left")
    df = df.dropna(subset=["rank"])
    print(f"学習データ: {len(df)}行")
    feature_cols = [
        "yoso_rank","car_no","frame_no","current_score","gear_ratio",
        ...
        "kimete_nige","kimete_makuri","kimete_sashi","kimete_ma","nige_rate","makuri_rate",
        "wins_s","wins_b","stadium_wins_year","last5years_at_stadium",
    ...
    return X, df["is_top1"].values, df["is_top3"].values, feature_cols, df
...
def train_model(X, y, label, feature_cols):
    ...
    for i in range(min(10, len(feature_cols))):
        print(f"    {i+1:2d}. {feature_cols[indices[i]]:30s}: {importances[indices[i]]:.4f}")
    return model

if __name__ == "__main__":
    print("="*60)
    print(f"競輪予想 改善版モデル {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("="*60)
    ...
    with open("keirin_model.pkl", "wb") as f:
        pickle.dump({"model_top1": model_top1, "model_top3": model_top3, "feature_cols": feature_cols}, f)
